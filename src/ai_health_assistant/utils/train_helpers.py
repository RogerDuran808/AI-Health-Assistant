...
):  # Si estem a l'entorn virtual no fa plot, ja que no em funciona
    import matplotlib
...
def train_models(
    ...
    train_report = classification_report(
        y_train, y_train_pred, labels=[0, 1], output_dict=True, zero_division=0
    )
    ...
    test_report = classification_report(
        y_test, y_test_pred, labels=[0, 1], output_dict=True, zero_division=0
    )

    # Print del report (resumit amb els valors més importants)
    print(
        f"\nTrain F1 (1): {train_report['1']['f1-score']:.4f} | "
        f"Test F1 (1): {test_report['1']['f1-score']:.4f} | "
...
def update_metrics_file(
    metrics: pd.DataFrame, filename="results/03_training/metrics.csv"
):
    columnas = [
        "Model",
        "Train F1 (1)",
...
def update_experiments_file(
    metrics: pd.DataFrame, filename="../results/02_experiments/experiments.csv"
):
    columnas = [
        "Experiment",
        "Train F1 (1)",
...
def save_model(best_estimator, model_name, save_external="no"):
    ...
    if save_external.lower() == "yes":
        ...
        joblib.dump(
            best_estimator, external_path
        )  # Guradem el model a la ruta externa, a la aplicació web
